...
export const UserBetsReportForm = () => {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      const response = await apiClient.requestUserBetsReport({
        userId,
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `user-bets-report-${userId}-${Date.now()}.csv`;
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        Generate a detailed CSV report of all bets placed by a specific user within a date range.
      </Typography>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, maxWidth: 600 }}>
        <Autocomplete
          ...
          disabled={isGenerating || usersLoading}
        ...
        <Box sx={{ display: 'flex', gap: 2, mt: 2 }}>
          <Button
            ...
            disabled={isGenerating || !userId || !startDate || !endDate}
